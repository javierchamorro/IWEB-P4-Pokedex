...
class RaceTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "\(pokedexModel.types[index].name)"
    }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return pokedexModel.types[index].races.count
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let type = pokedexModel.types[index]
        let race = type.races[indexPath.row]
        
        
        cell.textLabel?.text = race.name
        cell.detailTextLabel?.text = String(race.code)
        cell.imageView?.image = UIImage(named: "races/\(race.icon)")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "pokemons" {
            
            
            if let wvc = segue.destination as? WebViewController{
                
                if let index2 = tableView.indexPathForSelectedRow?.row {
                
                wvc.nombrePokemon = pokedexModel.types[index].races[index2].name
